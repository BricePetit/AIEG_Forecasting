...
from data.preprocessing import concat_production_sites
...
def load_data(
    ...
    sn_sites = [item.split('_')[2].split('/')[0] for item in sites]
    if "CHAMAIEG" in sites[0]:
        group_data = concat_production_sites(dataset, sn_sites) if len(sn_sites) > 1 else None
    else:
        group_data = pd.concat(
            [dataset[site].set_index('ts')[['ap']] for site in sites], axis=0
