...
def optimize_sites(productions_pred, consumption_pred):
    solver = pywraplp.Solver.CreateSolver('SCIP')
    vars = [solver.BoolVar(f'site_{i}') for i in range(len(productions_pred))]
    solver.Add(sum(vars[i] * productions_pred[i] for i in range(len(productions_pred))) <= consumption_pred)
    solver.Maximize(sum(vars[i] * productions_pred[i]))  # Maximiser production utilisée
    status = solver.Solve()
    if status == pywraplp.Solver.OPTIMAL:
        selected = [i for i, v in enumerate(vars) if v.solution_value() > 0.5]
        return selected
    return []
