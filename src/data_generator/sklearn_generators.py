...
__title__: str = "sklearn_generators"
...
class SkLearnDataGenerator(BaseDataGenerator):
    """
    Data generator for scikit-learn interface.
    """
    def __init__(
        ...
        """
        Constructor of the SkLearnDataGenerator class.
    ...
    def __getitem__(self, index: int) -> tuple:
        ...
        # windows_y = np.array(
        #     [self.data.iloc[x_stop: x_stop + self.out_length, [3, 4, 5]]], dtype='float32'
        # )
        # print(self.data.iloc[x_stop: x_stop + self.out_length, [3, 4, 5]])
        
        # windows_x = np.array([self.data.iloc[idx:x_stop]['ap']], dtype='float32')
        # windows_y = np.array(
        #     self.data.iloc[x_stop: x_stop + self.out_length]['ap'], dtype='float32'
        # )
        windows_x = np.array(
            [[self.data.iloc[i]['ap']] for i in range(idx,x_stop,1)], dtype='float32'
        )
        windows_y = np.array(
            [self.data.iloc[i]['ap'] for i in range(x_stop, x_stop + self.out_length, 1)],
            dtype='float32'
        )
        # windows_x = np.empty((self.batch_size, self.seq_length, 1), dtype='float32')
        # windows_y = np.empty((self.batch_size, self.out_length, 1), dtype='float32')
        # for i, batch_idx in enumerate(batch_indices):
        #     x_stop = batch_idx + self.seq_length
